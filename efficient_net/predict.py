...
model = load_model('efficientnet_mangrove_best.h5')
...
for class_idx in range(num_classes):
    # Ambil indeks dari 10 gambar kelas ini
    ...
    for i in class_indices:
        ...
        # Normalize ke range 0-1 jika perlu
        if img.max() > 1:
            img = img / 255.0
...
plt.tight_layout()
plt.suptitle("Hasil Prediksi Model - 5 Gambar per Kelas", fontsize=16, y=1.02)
plt.show()